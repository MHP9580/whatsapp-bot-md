	...
	async (message, match) => {
		...
		if (match == 'on' || match == 'off') {
			if (match == 'off' && !antilink)
				return await message.send('_AntiLink n''est pas établi''._')
			await setAntiLink(message.jid, match == 'on')
			return await message.send(
				`_AntiLink ${match == 'on' ? 'Activé' : 'Desactivé.'}_`
			)
		}
		if (match == 'info')
			return await message.send(
		...
		if (match.startsWith('action/')) {
			...
			return await message.send(`_AntiLink action mise à jour comme ${action}_`)
		}
		const res = await setAntiLink(message.jid, match)
		return await message.send(
			`_Les liens permis sont_\nPermis - ${res.allow.join(
				', '
			)}\nPasPermis - ${res.notallow.join(', ')}`
		)
	}
)
